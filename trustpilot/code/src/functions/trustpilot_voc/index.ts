...
export const run = async (events: any[]) => {
  for (const event of events) {
    ...
    if (parameters === 'help') { 
      ...
      const helpMessage = `trustpilot_voc - Fetch reviews from twitter and create tickets in DevRev.\n\nUsage: /trustpilot_voc  For getting the reviews`;
    ...
    let postResp: HTTPResponse = await apiUtil.postTextMessageWithVisibilityTimeout(snapInId, 'Fetching reviews from Trustpilot', 1);
    if (!postResp.success) {
      console.error(`Error while creating timeline entry: ${postResp.message}`);
      continue;
    }
    ...
    const url = 'https://trustpilot4.p.rapidapi.com/?domain=devrev.ai&page=1';
    const options = {
      method: 'GET',
      headers: {
